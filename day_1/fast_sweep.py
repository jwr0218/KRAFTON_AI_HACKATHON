"""빠른 스윕: 100 에폭, 조기종료, GPU 병렬"""
import torch, torch.nn as nn, torch.optim as optim
from torch.utils.data import Dataset, DataLoader
from torch.optim.lr_scheduler import CosineAnnealingLR
import random, time, sys, multiprocessing as mp
...
def run_one(args):
    ...
    for ep in range(max_epochs):
        ...
        if (ep+1) % 10 == 0:
            ...
            best = max(best, acc)
            if acc >= 0.99:
                return cfg, params, best, True, ep+1, time.time()-t0

    return cfg, params, best, False, max_epochs, time.time()-t0

if __name__ == '__main__':
    MAX_EPOCHS = 150

    # 가장 유망한 configs만 (params 오름차순)
    configs = [
        (8,  2, 2, 32),   # 1970
        (8,  2, 3, 32),   # 2842
        (16, 2, 2, 32),   # 4898
    ...
    # GPU 0, 1 교대 할당
    tasks = [(cfg, i % 2, MAX_EPOCHS) for i, cfg in enumerate(configs)]

    print(f"총 {len(configs)}개 config, max {MAX_EPOCHS} epochs, GPU 0/1 병렬\n")
    print(f"{'config':30s} {'params':>7} {'best':>7} {'ok':>5} {'ep':>5} {'sec':>6}")
    print("-"*60)

    # 순차 실행 (각 config 독립, 빠른 하드웨어면 충분)
    results = []
    for task in tasks:
        cfg, params, best, ok, ep, sec = run_one(task)
        d,h,l,f = cfg
        tag = "✓ 99%!" if ok else "✗"
        print(f"d{d} h{h} l{l} ff{f:3d}  {params:>7}  {best:.4f}  {tag:>5}  {ep:>5}  {sec:>5.0f}s", flush=True)
        results.append((cfg, params, best, ok, ep))

    print("\n=== 결과 ===")
    ok_results = [(p, cfg) for cfg, p, best, ok, ep in results if ok]
    if ok_results:
        p, cfg = min(ok_results)
        d,h,l,f = cfg
        print(f"최소 99% config: d_model={d}, n_heads={h}, n_layers={l}, d_ff={f}  ({p} params, 원본 대비 {p/39010*100:.1f}%)")
    else:
        print("99% 달성 config 없음 — epochs 더 필요")
